...
- 深度优先搜索
  - 如果扫到雷，标记结束。
  - 如果未扫到雷。
    - 计算当前坐标x,y周围雷的个数mines。
    - 如果mines为0，则当前坐标标记为'B'。对于周围标识为`E`的节点坐标做递归标记。(`重要`)
    - 如果mines为1-9, 标记，`不再扩展`。

- 广度优先搜索
  - 方法同深度优先遍历类似，只不过需要一个队列存储待处理的节点坐标。
  - 当前处理节点周围无雷且周边节点为E时，周边节点才入队列。
  - 需要一个辅助二维数组，标记已入过队列的节点坐标，防止重复进入队列。
